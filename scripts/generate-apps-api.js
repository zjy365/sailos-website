...
async function convertTemplateToAppConfig(template) {
  ...
  if (spec.draft === true) {
    console.log(
      `⏭ Skipping template "${spec.title || metadata.name}" - draft: true`,
    );
    return null;
  }
...
function getFileExtension(url) {
  try {
    ...
    if (
      ext &&
      ['.png', '.jpg', '.jpeg', '.svg', '.ico', '.webp', '.gif'].includes(
        ext.toLowerCase(),
      )
    ) {
      return ext;
  ...
  try {
    ...
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          '⚠️ Network unavailable. Skipping remote template refresh and keeping existing config.',
        );
        return;
      }
      throw error;
    ...
    if (addedCount > 0) {
      ...
      top10Apps.forEach((app, index) => {
        ...
        console.log(
          `  ${index + 1}. ${app.name} (${app.category}) - ${deployCount} deploys`,
        );
      });
    }
