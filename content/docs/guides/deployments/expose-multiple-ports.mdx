...
keywords: [multi-port application, expose multiple ports, Sailos platform, app launchpad, network configuration]
description: Learn how to expose multiple ports for complex applications using Sailos App Launchpad. Simplify network configuration and enhance accessibility.
---
...
on [Sailos](https://usw.sailos.io), you can easily choose to expose multiple ports. During the deployment process,
users simply click on the "Network" option and then select "Add Port" to configure multiple ports.

![](./images/sailos-add-port.png)

Furthermore, the Sailos platform offers external access to these ports. Once exposed to the public network, each port
