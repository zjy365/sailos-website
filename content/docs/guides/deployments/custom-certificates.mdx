...
Modify the ingress (replace xzy-dev.sailos.run with your own custom domain).

```bash
kubectl patch ingress network-twvuqmjzckur -p '{"spec":{"tls":[{"hosts":["xzy-dev.sailos.run"],"secretName":"network-test"}]}}'
```