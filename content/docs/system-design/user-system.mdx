...
Sealos 采用多租户架构设计，通过在公有云环境中实现多个用户共享同一 Kubernetes 集群来最大化资源利用率。用户系统正是为实现这一目标而构建，在确保多租户资源隔离的前提下，通过简洁优雅的设计方案平衡了系统安全性与用户体验。
...
![Sealos 多租户用户体系与命名空间映射关系示意图](./images/sealos-user-namespace-mapping.zh-cn.png)
...
用户系统主要代码放在 [controllers/user](https://github.com/labring/sealos/tree/main/controllers/user) 部分，包含自定义资源的定义与控制器。整体采用 KubeBuilder 生成。

用户创建等操作相关的主要逻辑放在 [controllers/user/controllers/user_controller.go](https://github.com/labring/sealos/blob/main/controllers/user/controllers/user_controller.go) 部分。其 Reconcile 方法的核心流程如下，由一系列函数组成：
