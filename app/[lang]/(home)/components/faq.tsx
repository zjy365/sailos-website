      ...
      <AnimateElement type="slideUp">
        ...
        <div className="mx-auto mt-16 max-w-3xl">
          <Accordion type="single" collapsible defaultValue="item-0" className="w-full">
            {t.faqs.map((faq, index) => (
              ...
              >
                <AccordionItem
                  value={`item-${index}`}
                  className="group overflow-hidden rounded-lg border border-gray-200 bg-white px-6 mb-4 shadow-sm transition-all duration-200 hover:border-gray-300 hover:shadow-md"
                >
                  <AccordionTrigger className="text-left text-base font-semibold hover:no-underline py-5">
                    <div className="flex items-center justify-between w-full">
                      <span>{faq.question}</span>
                      <div className="flex-shrink-0 ml-2 text-gray-400 group-data-[state=open]:rotate-45 transition-transform duration-200">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <p className="text-lg font-medium mb-4">{t.stillHaveQuestions}</p>
            <a
              href="mailto:contact@sealos.io"
              className="text-[#44BCFF] hover:text-[#0090FF] font-medium transition-colors duration-200"
            >
              {t.contactUs} →
            </a>
          </motion.div>
